...
# data_file = 'receiver-clam-10Mb-55ms.result'
data_file = 'receiver-localhost-10Mb-55ms.result'

# title = '1Gb LAN'
# title = '''Atlanta - Madison
# 10Mb, 55ms'''
title = '''localhost and tc
10Mb, 55ms, burst 100Mb'''
...
grouped_df = df.groupby(['test name', 'number of bytes']).mean(
...
for i, test_name in enumerate(test_names):
    ...
    test_series = grouped_df[grouped_df['test name'] ==
                             test_name][['wall clock us', 'number of bytes']]
    print(test_series)

    # Calculate the x positions for the bars in the current group
    x_positions = [i * (bar_width + spacing) +
                   j for j in range(len(data_sizes))]

    # Create the bars for the current group
    ax.bar(x_positions, test_series['wall clock us'],
           width=bar_width, label=test_name)

# Set the x ticks and labels for each group of bars
ax.set_xticks([i + (len(data_sizes) - 1) / 2 * (bar_width + spacing)
